...
`檢測參數設定` 內新增 `尺寸良品判斷條件` 分頁。這些規則屬於各自的檢測參數，不是全域共用清單；切換不同檢測參數時，應只看到該參數自己的規則。

目前已完成：

- 每個規則可設定名稱、啟用狀態、`A 規` 與 `B 規` 的計算式及規格式。
- 計算式支援量測紀錄編號參照，例如 `(1)`、`(2)`，以及基本四則運算、`min`、`max`。
- 規格式支援單值、範圍與比較條件，並在保存前檢查計算式與規格式是否成對且格式有效。
- 規則表格顯示編號、名稱、啟用狀態、A/B 規內容；可新增、修改、清除、上移、下移、刪除及保存。
- 規則右鍵選單提供修改、上移、下移、刪除；清單順序變更不會改變規則的唯一 ID。
- 每筆規則都有獨立 GUID 與保存編號。名稱或順序可以變更，跨資料引用仍應使用 GUID，不可使用顯示名稱。
- `SystemParameters.ini` 已保存每個檢測參數的規則數量、規則 ID、編號、名稱、啟用狀態、A/B 規計算式與規格式；舊設定載入時會補齊缺少的 ID 與編號。

目前尚未完成：

- 尚未把保存的 A/B 規直接接到實際量測結果，尚未自動產生 `A規`、`B規`、`C規` 或 `不可判斷` 的最終判定。
- `參數結果確認` 仍需接上量測資料與規則評估結果；規則編輯器目前負責設定、驗證與保存。
- 後續評估流程應只讀取已完成且快取有效的量測結果，不應因切換分頁或重新選取規則而重跑前處理、影像處理、區塊處理或量測線。

建議的後續串接順序：

1. 由尺寸量測結果建立「量測編號 -> 實際數值」的只讀快照。
2. 依啟用規則解析計算式與規格式，產生每個規則的通過/不通過/資料不足結果。
3. 按專案定義整合 A/B 規，將結果寫入 `參數結果確認`，並保留使用的規則 ID、量測紀錄 ID 與評估時間。
4. 當量測紀錄刪除或編號來源失效時，在規則表與結果頁指出失效引用，不可靜默改用其他量測線。
