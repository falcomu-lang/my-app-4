...
- 物件定義：位於整合成區塊下方，可建立多個物件組定義；目前先保存物件組 ID/名稱與未來的區塊引用欄位。
...
9. 在 `物件定義` 上按右鍵新增 `物件組定義`；物件組的區塊選擇與最終判定規則將在後續流程接入。
10. 點擊區塊可展開/收合其處理步驟；選取兩個以上未分組區塊後按右鍵，可建立物件群組；選取兩個以上群組後按右鍵，可再建立上層群組。
...
6. partial 拆分穩定後，再評估把純 OpenCV 函數抽至 `Services/OpenCvProcessingService.cs`。
