...
以 C# Windows Forms 開發的高解析度影像檢視與 ROI 影像處理工具。軟體以灰階影像處理為核心，支援大尺寸圖片的切圖顯示、ROI 管理，以及 OpenCV Canny 邊緣偵測結果的即時檢視。
...
- Canny 結果以完整解析度 OpenCV Mask 保存；放大、平移與重設視圖不會重新執行 Canny。
...
- `影像處理時間`：Gaussian Blur 與 Canny 時間。
